...
{
    public class LzwFormsPrincipal<TUserData> : IPrincipal
    where TUserData : class, new()
    {
        private IIdentity _identity;
        private TUserData _userData;

        public LzwFormsPrincipal(FormsAuthenticationTicket ticket, TUserData userData)
        {
            if (ticket == null)
        ...
        /// <summary>
        /// 尝试从HttpRequest.Cookies中构造一个LzwFormsPrincipal对象
        /// </summary>
        /// <param name="request"></param>
        /// <returns></returns>
        public static LzwFormsPrincipal<TUserData> TryParsePrincipal(HttpRequest request)
        {
            if (request == null)
            ...
            try
            {
                ...
                if (ticket != null && userData != null)
                    // 4. 构造我们的LzwFormsPrincipal实例，重新给context.User赋值。
                    return new LzwFormsPrincipal<TUserData>(ticket, userData);
            }
        ...
        public static void TrySetUserInfo(HttpContext context)
        {
            ...
            LzwFormsPrincipal<TUserData> user = TryParsePrincipal(context.Request);
            if (user != null)
                context.User = user;
        }
    }
